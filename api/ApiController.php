...
class ApiController extends EasyApi
{
    ...
    protected function __init()
    {
        if (empty($this->request->input('apiData'))) {
            throw new NormalStatusException(t('easycmf.access_denied'), EasyCode::NORMAL_STATUS);
        }
    ...
    public function v1(): ResponseInterface
    {
        ...
        try {
            ...
            if ($e instanceof ValidationException) {
                ...
                throw new NormalStatusException(t('easycmf.interface_exception') . $error, EasyCode::INTERFACE_EXCEPTION);
            }
            if ($e instanceof NoPermissionException) {
                throw new NormalStatusException(t(key: 'easycmf.api_auth_fail') . $e->getMessage(), code: EasyCode::NO_PERMISSION);
            }
            if ($e instanceof TokenException) {
                throw new NormalStatusException(t(key: 'easycmf.api_auth_exception') . $e->getMessage(), code: EasyCode::TOKEN_EXPIRED);
            }

            throw new NormalStatusException(t(key: 'easycmf.interface_exception') . $e->getMessage(), code: EasyCode::INTERFACE_EXCEPTION);
        }
    }
