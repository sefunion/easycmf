...
use Easy\Generator\ApiGenerator;
use Easy\Generator\ControllerGenerator;
use Easy\Generator\DtoGenerator;
use Easy\Generator\MapperGenerator;
use Easy\Generator\ModelGenerator;
use Easy\Generator\RequestGenerator;
use Easy\Generator\ServiceGenerator;
use Easy\Generator\SqlGenerator;
use Easy\Generator\VueIndexGenerator;
use Easy\Easy;
...
class SettingGenerateTablesService extends AbstractService
{
    ...
    {
        // 非系统数据源，同步远程库的表结构到本地
        if ($params['source'] !== Easy::getEasyName()) {
    ...
    protected function generateCodeFile(int $id, int $adminId): SettingGenerateTables
    {
        ...
        foreach ($classList as $cls) {
            $class = make($cls);
            if (get_class($class) == 'Easy\Generator\SqlGenerator') {
    ...
    protected function packageCodeFile(): string
    {
        ...
        $zipFileName = BASE_PATH . '/runtime/easycmf.zip';
