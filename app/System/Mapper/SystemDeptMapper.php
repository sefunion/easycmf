...
class SystemDeptMapper extends AbstractMapper
{
    ...
    public function getSelectTree(): array
    {
        ...
        $deptTree = (new EasyCollection())->toTree($treeData, $treeData[0]['parent_id'] ?? 0);

        if (config('easycmf.data_scope_enabled', true) && ! user()->isSuperAdmin()) {
            ...
            return (new EasyCollection())->toTree(array_merge($treeData, $deptTree), $treeData[0]['parent_id'] ?? 0);
        }
        return $deptTree;
    }
    ...
    public function getLeaderList(?array $params = null): array
    {
        if (empty($params['dept_id'])) {
            throw new EasyException('缺少部门ID', 500);
        }
