...
class SystemUploadFileService extends AbstractService
{
    ...
    protected EasyUpload $easyUpload;

    public function __construct(SystemUploadFileMapper $mapper, EasyUpload $easyUpload)
    {
        $this->mapper = $mapper;
    ...
    public function upload(UploadedFile $uploadedFile, array $config = []): array
    {
        ...
        $data = $this->easyUpload->upload($uploadedFile, $config);
        if ($this->save($data)) {
            return $data;
        }
        return [];
    ...
    public function chunkUpload(array $data): array
    {
        ...
        $result = $this->easyUpload->handleChunkUpload($data);
    ...
    {
        $data = $this->easyUpload->handleSaveNetworkImage($data);
    ...
    public function responseFile(string $hash): ResponseInterface
    {
        ...
        return container()->get(EasyResponse::class)->responseImage(
            $this->easyUpload->getFileSystem()->read(
                $this->easyUpload->getStorageMode() === '1'
